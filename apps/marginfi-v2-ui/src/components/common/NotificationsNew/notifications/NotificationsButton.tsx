...
import { ChannelType, ThemeType } from "@dialectlabs/react-ui";
...
const NotificationsButtonPresentation = ({ theme, children }: PropsWithChildren<{ theme?: ThemeType }>) => {
  return (
    <div className="dialect" data-theme={theme}>
      <div className={clsx("dt-relative", ClassTokens.Text.Primary)}>{children}</div>
    </div>
  );
};
...
NotificationsButtonPresentation.Container = function NotificationsButtonContainer({
  channels,
  ...
  const externalProps = useMemo(() => ({ open, setOpen, channels, theme }), [open, channels, theme]);
  ...
  return (
    <NotificationsButtonPresentation theme={theme}>
      ...
      {children ? (
        ...
        <DefaultNotificationIconButton ref={buttonRef} open={open} onClick={toggle} unread={unreadCount > 0} />
      )}
      {/* Modal Render */}
      {renderModalComponent ? (
        renderModalComponent({
