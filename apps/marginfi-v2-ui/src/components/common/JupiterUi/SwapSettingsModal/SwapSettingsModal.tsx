...
import { detectedSeparator, formatNumber } from "~/utils";
import {
  DEFAULT_SLIPPAGE,
  PRIORITY_HIGH,
...
import { InformationMessage, Toggle } from "../Common";
...
export const SetSlippage: React.FC<{ closeModal: () => void }> = ({ closeModal }) => {
  ...
  const [isStrictTokenList, setIsStrictTokenList] = useJupiterStore((state) => [
    state.isStrictTokenList,
    state.setIsStrictTokenList,
  ]);
  ...
  const isStrictTokenListInput = form.watch("isStrictTokenList");
  ...
  const onClickSave = () => {
    ...
    setIsStrictTokenList(isStrictTokenListInput);
    closeModal();
  };
  ...
  return (
    <div className="w-full rounded-xl flex flex-col bg-jupiter-bg text-white shadow-xl max-h-[90%]">
      ...
      <form
        ...
        className="relative w-full overflow-y-auto webkit-scrollbar overflow-x-hidden"
      >
        <div>
          <div className="mt-2 px-5">
            {/**************************** PRIORTY *****************************/}
            <Typography className="flex items-center text-sm text-white/75 font-[500]" gutterBottom component="div">
            ...
            <div className="flex items-center justify-between mt-2">
              ...
              <Toggle
                active={isStrictTokenList}
                onClick={() => form.setValue("isStrictTokenList", isStrictTokenList ? false : true)}
              />
            </div>
            <p className="mt-2 text-xs text-white/50">
          ...
          <div className="px-5 pb-5">
            <button type="button" onClick={onClickSave} className={"w-full mt-4"} disabled={isDisabled}>
